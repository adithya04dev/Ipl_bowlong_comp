...
def main():
    # Title of the app
    st.title("IPL: Bowlers Comparision ")
    ...
    if st.button('Submit'):
        ...
        result=bowcomp.calculateb(overs,batting_type,Season,min_balls)
        ...
        for i, row in result1.iterrows():
            scatter_plot.text(row['runrate'],row['average'], row['player_name'], fontsize=18, alpha=0.7)
